function m = Ning_g_data2fit(signal, xps, opt)
% m = [MD V k S0]
...
t_ub       = [4.000 20.000 40.0 2*ones(size(S0))];

unit_to_SI = [1e-9 1e-18 1.0 S0];

    function s = fun(t,varargin)
        m = t .* unit_to_SI;
        s = Ning_g_fit2data(m, xps);
    end

if opt.n_inits > 0
    t = msf_fit(@fun, signal, t_lb, t_ub, opt.n_inits, opt.Ning_g.lsq_opts);
else
    t = lsqcurvefit(@fun, t_guess, [], signal, ...
        t_lb, t_ub, opt.Ning_g.lsq_opts);
end
m = t .* unit_to_SI;


if (opt.Ning_g.do_plot)
    signal_fit = Ning_g_fit2data(m, xps);
    x = (1:numel(signal))';
    figure(1),clf
