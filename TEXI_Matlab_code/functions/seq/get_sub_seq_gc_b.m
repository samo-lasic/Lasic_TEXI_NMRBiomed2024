function [sub_ind, sub_seq] = get_sub_seq_gc_b(seq, nc, b_min, b_max)
% b limits in s/mm^2
b_min = b_min * 1e6; % SI units
b_max = b_max * 1e6; % SI units

sub_ind = find(seq.gc_array(nc) == seq.gc & seq.b >= b_min & seq.b <= b_max);
...
% warrning: delta_array and g_array are copied from the original and may not be correct (since they are not used in the fitting there is no problem)

ind = seq.tm_array >= min(sub_seq.tm) & seq.tm_array <= max(sub_seq.tm);
sub_seq.tm_array = seq.tm_array(ind);
sub_seq.ntm = length(unique(sub_seq.tm_ind));
